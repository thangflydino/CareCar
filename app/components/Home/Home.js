...
const Home = ({route}) => {
  ...
  const listImage = [
    'https://i.ibb.co/NmBdF7f/Banner.png',
    'https://i.ibb.co/CWRM5L3/41759a96bdac71f228bd.jpg',
  ];
  const listCategory = [
    {
      name: 'Gọi cứu hộ',
  ...
  const handlePressCategory = type => {
    ...
    } else if (type === 'CallForHelp') navigation.navigate('CallForHelp');
    else if (type === 'Voucher') navigation.navigate('Voucher');
    else if (type === 'CarWash') navigation.navigate('CarWash');
  };
  useEffect(() => {
    CommonAPIs.getAllGarages()
      .then(res => {
      ...
      <>
        <View style={styles.search}>
          <TouchableOpacity
            ...
            onPress={() =>
              navigation.navigate('SearchGarage', {listGarages: listGarages})
            }>
          ...
          <TouchableOpacity
            ...
            <Text style={[styles.textSearch, {color: 'black'}]}>
              {location}
            </Text>
        ...
        <View style={styles.listService}>
          {listGarages.map((item, index) => (
            <TouchableOpacity
              ...
              key={item.id}
              onPress={() =>
                navigation.navigate('DetailGarage', {idGarage: item.id})
              }>
              <Image
