...
const listService = [
  ...
  },
  {
    ...
    description:
      '- Chẩn đoán, đọc, xóa lỗi ô tô thế hệ mới - Hệ thống phun xăng điện tử EFI, GDI - Sửa chữa, bảo dưỡng, đại tu Máy – Gầm – Điện - Điện - Điện lạnh - Hỗ trợ sửa chữa lưu động, sửa chữa tại nhà, cơ quan',
  },
  {
    name: 'Đồng sơn ô tô',
    description:
      '- Gò, Hàn, Sơn ,phục hồi xe tai nạn. - Sơn La zăng. - Sơn chóe đèn ô tô. - Sơn đổi màu, sơn theo yêu cầu.',
  },
];
const time = [
  '07:00',
...
const MakeAnAppointment = ({route}) => {
  ...
  const handleOnNext = () => {
    switch (step) {
      case 1:
        if (!garageName) Alert.alert('Thông báo', 'Bạn chưa chọn garage');
        else if (!date) Alert.alert('Thông báo', 'Vui lòng chọn ngày hẹn');
        else if (!timeAppointment)
          Alert.alert('Thông báo', 'Vui lòng chọn giờ hẹn');
        else setStep(step + 1);
        break;
      case 2:
        if (selected.length > 0) setStep(step + 1);
        else Alert.alert('Thông báo', 'Vui lòng chọn dịch vụ');
        break;
      case 3:
  ...
  const handleOnChoiceDate = () => {
    if (!garageName) Alert.alert('Thông báo', 'Vui lòng chọn Garage');
    else navigation.navigate('ChoiceDate', {nameGarage: garageName});
  };
  return (
    <View style={styles.container}>
      ...
      <View style={styles.procedure}>
        {procedure.map((item, index) => (
          <View style={styles.procedureItem} key={item.id}>
            <View
              style={[
                styles.procedureItemCount,
                step >= index + 1 && styles.vRed,
              ]}>
              <Text style={styles.procedureItemCountText}>{item.count}</Text>
            </View>
            <Text
              style={[
                styles.procedureItemTitle,
                step >= index + 1 && styles.textRed,
              ]}>
              {item.title}
            </Text>
          </View>
      ...
      {step == 1 && (
        <View style={styles.content}>
          <TouchableOpacity
            style={styles.choiceGarage}
            onPress={() => navigation.navigate('ChoiceGarage')}>
            <Icon name="home" size={30} color="red" />
            <Text style={[styles.garageName, garageName && {color: 'black'}]}>
              {garageName || 'Chọn garage'}
            </Text>
          </TouchableOpacity>
          ...
          <View style={styles.dateContent}>
            <Text style={styles.date}>{date || 'Chưa chọn ngày hẹn'}</Text>
            <TouchableOpacity
              ...
              <FlatList
                ...
                renderItem={({item}) => (
                  <TouchableOpacity
                    style={[
                      styles.itemTime,
                      timeAppointment == item && styles.vRed,
                    ]}
                    onPress={() => setTimeAppointment(item)}>
                    <Text
                      style={[
                        styles.timeText,
                        timeAppointment == item && styles.textWhite,
                      ]}>
                      {item}
                    </Text>
                  </TouchableOpacity>
